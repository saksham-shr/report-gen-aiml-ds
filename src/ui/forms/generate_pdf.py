...
from ...models.report import ActivityReport
...
<body>
    ...
    <!-- University Header -->
    <div class="university-title">{UNIVERSITY_INFO['name']}</div>
    <div class="school-department">{UNIVERSITY_INFO['school']}</div>
    <div class="school-department">{UNIVERSITY_INFO['department']}</div>
    ...
    <div class="field-value">Total Participants: {total_participants}</div>
        ...
        if activity.get('highlights') or activity.get('key_takeaway'):
            html += """
    ...
    <div class="paragraph">{activity.get('highlights')}</div>
                """
            if activity.get('key_takeaway'):
                html += f"""
    <div class="field-label">Key Takeaways:</div>
    <div class="paragraph">{activity.get('key_takeaway')}</div>
                """

        # Add report preparers
        html += """
    <div class="section-title">Report Prepared By</div>
        """
        for preparer in activity_data.get('report_preparers', []):
            html += f"""
    <div class="field-value">{preparer.get('name', 'N/A')}, {preparer.get('designation', 'N/A')}</div>
            """
...
"""
    ...
    def preview_report(self):
        ...
        QMessageBox.information(self, "Preview", "Preview functionality would show the formatted report in a web view.")
